...
export function AdminApp() {
  ...
  const [form, setForm] = useState<MasterclassFormState>(() =>
    ...
    try {
      ...
      const detailed = await Promise.all(
        files.map(async (file) => {
          const { product, sha } = await getMasterclass(currentToken.trim(), file.name);
          return {
            name: file.name,
            slug: product.slug,
            title: product.title,
  ...
  function startCreate() {
    ...
    setSlugLocked(false);
    setForm(emptyMasterclassForm(adminConfig.covers[0]));
    setMessage(null);
    setError(null);
  }
  ...
  async function handleSave(event: React.FormEvent) {
    ...
    try {
      const product = formToProduct(form, { publishedAt: editingPublishedAt });
      setLoading(true);
  ...
  async function handleDelete(item: ListItem) {
    if (!confirm(`Удалить мастер-класс «${item.title}»?`)) return;
    setLoading(true);
    setError(null);
    try {
      ...
      <section className="section-padding pt-32">
        <Container size="narrow">
          ...
          <p className="mt-3 text-muted">
            Вход только для управления мастер-классами.
          </p>
          <form onSubmit={handleLogin} className="mt-8 space-y-4 rounded-3xl border border-border bg-card p-6">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-muted">
              Добавление и редактирование мастер-классов. После сохранения GitHub
              сам обновит сайт.
            </p>
          </div>
        ...
        {mode === "list" ? (
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-2xl">Мастер-классы</h2>
              <Button type="button" onClick={startCreate} disabled={!canSave || loading}>
            ...
            {!canSave ? (
              ...
              <p className="text-muted">Пока нет мастер-классов.</p>
            ) : (
          ...
          <form onSubmit={handleSave} className="space-y-6 rounded-3xl border border-border bg-card p-6 md:p-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-2xl">
                {mode === "create" ? "Новый мастер-класс" : "Редактирование"}
              </h2>
              <Button
                type="button"
                variant="ghost"
